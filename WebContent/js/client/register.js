...
var cb = function(){
	...
	$('#reset-btn').click(function() {
		...
		$("#persid").removeAttr("readonly", "readonly");
		$("#tel").removeAttr("readonly", "readonly");
		$("#email").removeAttr("readonly", "readonly");
		$("#address").removeAttr("readonly", "readonly");
		$("#introducer").removeAttr("readonly", "readonly");
		$("#allergicHis").removeAttr("readonly", "readonly");
...
function selectPatient() {
	...
	$('#patient-table').bootstrapTable({
		...
		pagination : true,
		queryParamsType : '', // 默认值为 'limit' ,在默认情况下 传给服务端的值为：offset
		// limit sort
		...
		onClickRow : function(row, tr) {
			...
			$("#introducer").attr("readonly", "readonly");
			$("#allergicHis").val(row.allergicHis);
			$("#allergicHis").attr("readonly", "readonly");
			...
			$("#dentistId option[value='" + row.maindocId + "']").attr("selected", true);// 根据值让option选中
			$("#maindocName").val(row.maindocName);// 根据值让option选中
			$("#maindocId").val(row.maindocId);// 根据值让option选中

			$('#select-patient-content').modal("hide");

			// checkbox默认选中
			var strNewlyAsk = row.newlyAsk;
