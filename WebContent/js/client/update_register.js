...
// 获取叫号参数
function getCallParams(calltype, registerId) {
	$.ajax({
		url : "/enjoyhisfy/client/register/get_call_params.json",
		data : {
			"calltype" : calltype,
			"type" : 5,
		...
		dataType : 'json',
		success : function(data) {
			// console.info(data.returndata);
			var parmas = data.returndata;
			$.ajax({
				url : "http://" + callNoIp + "/api/his/v1/helper/sign/verify",
				data : {
					"str" : parmas.uniqid
				},
				type : 'post',
