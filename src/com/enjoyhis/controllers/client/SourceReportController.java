...
public class SourceReportController {
	...
	@RequestMapping(value = "gotoSourceReport.htm")
	public String gotoSourceReport(Model model, HttpServletRequest request, HttpServletResponse response, Long id) {
		...
		return "report/source_report";
	}

	@RequestMapping(value = "/sourceReport.json")
	public void sourceReport(HttpServletRequest request, HttpServletResponse response, String strDate, String endDate, Long docId, Integer isnew) {
		PageResultForBootstrap page = new PageResultForBootstrap();
		Map<String, Cookie> cookieMap = new HashMap<String, Cookie>();
		Cookie[] cookies = request.getCookies();
		if (null != cookies) {
		...
		if (he.getUserType() != 1) {
			...
			page.setRows(sourceReportService.selectDateList(strDate, endDate, docId, isnew));
		} else {
			page.setRows(sourceReportService.selectDateList(strDate, endDate, loginId, isnew));
		}
		request.setAttribute(Constants.pageResultData, page);
	}
	@RequestMapping(value = "sourceReportToExcell.htm", method = RequestMethod.GET)
	public ResponseEntity<byte[]>  sourceReportToExcell(HttpServletRequest request, HttpServletResponse response, String strDate, String endDate, Long docId, Integer isnew) throws Exception {
		PageResultForBootstrap page = new PageResultForBootstrap();
		Map<String, Cookie> cookieMap = new HashMap<String, Cookie>();
		Cookie[] cookies = request.getCookies();
		if (null != cookies) {
...
//		request.setAttribute(Constants.pageResultData, page);
		...
		headers.setContentDispositionFormData("attachment", new String(("来源统计表.xlsx").getBytes("gb2312"), "iso-8859-1"));
		byte[] bytes = sourceReportService.getCardDetailListExcel(page.getRows());
